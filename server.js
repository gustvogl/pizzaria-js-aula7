// =============================================================
// server.js — Servidor Principal da API da Pizzaria Online
// =============================================================
...
// cors: permite que o Front-end (em outro domínio/porta) acesse nossa API
...
// Logger: registra no terminal todo pedido que chega
const logger = require('./middlewares/logger');

// ErrorHandler: captura qualquer erro não tratado nas rotas (o "seguro" da pizzaria)
const errorHandler = require('./middlewares/errorHandler');


// ─── 3. Criação da Aplicação Express ─────────────────────────
// app é o nosso "servidor" ou "cozinha". É nele que registramos middlewares e rotas.
const app = express();
...
// Sem isso, req.body seria undefined em rotas de POST (novo pedido) e PUT.
...
app.get('/', (req, res) => {
    res.json({ mensagem: '🍕 Bem-vindo à API da Pizzaria Online! (Aula 6)' });
});
...
//
// Exemplo: GET /api/hamburguer → cai aqui!
// =============================================================
app.use((req, res, next) => {
    res.status(404).json({
        sucesso: false,
        mensagem: `A rota '${req.url}' não existe no cardápio da Pizzaria.`
    });
});
