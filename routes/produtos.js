...
// =============================================================
// O que é CRUD?
//   Create (POST)   → Criar produto novo (Assar nova pizza)
//   Read   (GET)    → Ler/listar produtos (Ver o cardápio)
//   Update (PUT)    → Atualizar produto existente (Mudar o preço)
//   Delete (DELETE) → Remover produto (Tirar do cardápio)
//
...
router.get('/', (req, res) => {
    ...
    // Se o parâmetro foi enviado, filtramos as pizzas/bebidas por categoria
    if (categoriaId) {
    ...
    // Se não tem filtro, retorna todo o cardápio
...
router.get('/:id', (req, res) => {
    ...
    if (produto) {
        ...
        res.status(404).json({ mensagem: 'Pizza/Produto não encontrado no cardápio.' });
    }
});
...
//   { "categoriaId": 1, "nome": "Pepperoni", "descricao": "Mussarela e pepperoni fatiado", "preco": 55.00, "imagem": "pepperoni.png" }
//
...
router.put('/:id', (req, res) => {
    ...
    if (index !== -1) {
        ...
        res.status(404).json({ mensagem: 'Pizza/Produto não encontrado para atualização.' });
    }
});
